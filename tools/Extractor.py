...
class Extractor:
    ...
    def parseXML(self):
        ...
        try:
            ...
            split_pages_xmlfile = self.INPUT_PATH[:self.INPUT_PATH.rindex('.')] + '.split.xml'
            ...
            for p_num, p in split_pages.items():
                # get the image file of the scanned page
                imgfilebasename = p['image'][:p['image'].rindex('.')]
                imgfile = os.path.join(self.INPUT_DIR,p['image'])
            ...
            repaired_xmlfile = os.path.join(self.OUTPUT_DIR, output_files_basename + '.split.repaired.xml')
    ...
    def set_check_split_pages(self, split):
        self.check_split_pages = split
